...
export const deleteBoard = async (id) => {
	...
	/* 	const board = await Board.findById(owner._id);
	cons ole.log("column delete>>board cards", owner, cards);

	if (board !== null) {
		// remove from owner's list
		await board.content.pull(id);
		await board.save();
		con/sole.log("column delete>>board saved", board);
	} else {
		conso/le.log("Own er document is deleted");
	} */
	try {
		content.forEach((el) => {
		...
		console.error("No column found");// FIXME: ???
	}
